...
const DepartmentNewForm = ({ open, handleClose }: DepartmentNewFormProps) => {
  ...
  useEffect(() => {
    if (formik.values.latitude && formik.values.longitude) {
      setMapUrl(
        `https://yandex.ru/map-widget/v1/?ll=${formik.values.longitude}%2C${formik.values.latitude}&z=16&pt=${formik.values.longitude},${formik.values.latitude},pm2rdl`,
      );
    } else {
      setMapUrl('');
    }
...
export default observer(DepartmentNewForm);
